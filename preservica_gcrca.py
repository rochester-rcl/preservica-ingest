...
for dsstore in tiff_path.rglob('*DS_Store'):
    dsstore.unlink()
# Lines 26-30 looks through everything in the folder containing the TIFF files, finds each TIFF file, then adds the parent folder to a list if it isn't already in the list
asset_dirs = list()
tiff_files = tiff_path.rglob('*.tif*')
for file in tiff_files:
    if file.parent not in asset_dirs:
        asset_dirs.append(file.parent)
# Line 32 starts looping through the list of folders that will become assets in Preservica, each of which contains all the TIFFs for the asset
for asset in asset_dirs:
# Lines 34-35 we create a new subdirectory called "Representation_Preservation" inside the folder we are currently working on
    rep_pres_path = asset.joinpath('Representation_Preservation')
    rep_pres_path.mkdir()
# Lines 37-41 we find all the TIFF files in the current folder, create a lot of new subdirectories in "Representation_Preservation" that are each named the same as the TIFF file, and then move the corresponding TIFF file into that subdirectory just for the specific file 
...
# Lines 51-54 take the contents of our asset folder, which contains both "Representation" subdirectories which contain 1 PDF and many TIFFs and puts them into a new zip file within our "opex" folder - the zip file is named the same as our asset folder with a file extension of ".pax.zip" at the end. We now have our zipped PAX objects
    pax_obj = ZipFile(upload_path.joinpath(asset.name + '.pax.zip'), 'w')
    for entity in asset.rglob('*'):
        pax_obj.write(entity, arcname = entity.relative_to(asset))
    pax_obj.close()
# Line 56 now starts looping through all of the newly created zip files from the previous section - Line 57 pulls the "title" field by grabbing the zip files's file name - Line 58 is whatever security tag you would like to apply to the resource
for pax_obj in upload_path.glob('*'):
    title = str(pax_obj.name).split('.')[0]
    sec_tag = 'public'
# Lines 60-63 open the zip file, generate a SHA1 checksum for it, store it in a variable, and close the zip file again
    pax_hand = open(pax_obj, 'rb')
    pax_read = pax_hand.read()
    sha1_checksum = hashlib.sha1(pax_read).hexdigest()
    pax_hand.close()
# Line 65 starts working on the Dublin Core metadata, creating a string that we will keep adding to which starts with the XML record header
...
# Lines 66-70 opens up the metadata spreadsheet we identified on line 14, creates a CSV reader to analyze it, and then starts looping through every line in the spreadsheet. Since the first column of the spreadsheet contains the same number that is both the asset folder name, as well as our zipped PAX object name, when we match the "title" we identified on line 57 to a line in the spreadsheet, we know we've found a match and use that line to generate the metdata
    with open(md_path, 'r', encoding='utf8', newline='') as md:
        csv_reader = csv.reader(md, quotechar='"', delimiter=',')
        for row in csv_reader:
            if row[0] == title:
# Lines 72-101 are all pretty identical, for a given column of the spreadsheet, first we check to see if the cell is empty or not, and if it contains data, then we add a line to the running Dublin Core record started on line 65 with the relevant field, then one line 102 we add the closing tag for the Dublin Core record and now our descriptive metadata is done
                if row[14] != '':
                    dublin_core += f'\n\t\t\t<dc:title>{row[14]}</dc:title>'
                if row[9] != '':
                    dublin_core += f'\n\t\t\t<dc:title>{row[9]}</dc:title>'
                if row[3] != '':
                    dublin_core += f'\n\t\t\t<dc:creator>{row[3]}</dc:creator>'
                if row[13] != '':
                    dublin_core += f'\n\t\t\t<dc:subject>{row[13]}</dc:subject>'
                if row[5] != '':
                    dublin_core += f'\n\t\t\t<dc:description>{row[5]}</dc:description>'
                if row[10] != '':
                    dublin_core += f'\n\t\t\t<dc:publisher>{row[10]}</dc:publisher>'
                if row[1] != '':
                    dublin_core += f'\n\t\t\t<dc:contributor>{row[1]}</dc:contributor>'
                if row[4] != '':
                    dublin_core += f'\n\t\t\t<dc:date>{row[4]}</dc:date>'
                if row[15] != '':
                    dublin_core += f'\n\t\t\t<dc:type>{row[15]}</dc:type>'
                if row[6] != '':
                    dublin_core += f'\n\t\t\t<dc:format>{row[6]}</dc:format>'
                if row[7] != '':
                    dublin_core += f'\n\t\t\t<dc:identifier>{row[7]}</dc:identifier>'
                if row[12] != '':
                    dublin_core += f'\n\t\t\t<dc:source>{row[12]}</dc:source>'
                if row[8] != '':
                    dublin_core += f'\n\t\t\t<dc:language>{row[8]}</dc:language>'
                if row[2] != '':
                    dublin_core += f'\n\t\t\t<dc:coverage>{row[2]}</dc:coverage>'
                if row[11] != '':
                    dublin_core += f'\n\t\t\t<dc:rights>{row[11]}</dc:rights>'
    dublin_core += '\n\t\t</oai_dc:dc>\n\t'
# Lines 104-116 are a template for the OPEX metadata that we start plugging values into, including the title, the security tag, the checksum, and the entire Dublin Core record - OPEX metadata can be a wrapper for whole other metadata schemas
...
# Lines 118-119 open up a new file that is named the same as the zip file, except with ".opex" on the very end to indicate that this is the metadata for that zip file, and then writes the data into it
    opex_path = upload_path.joinpath(title + '.pax.zip.opex')
    opex_path.write_text(opex, encoding='utf8', newline='')
# Line 8 established a start time for the project, while lines 121-123 create the end time, figure out how much time has elapsed, and prints that out in minutes to the console
end_time = time.time()
